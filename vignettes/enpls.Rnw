...
\caption{Feature selection in \texttt{enpls}} \label{fig:diagram_enpls_fs}
\end{figure}
...
\section{Ensemble PLS for Measuring Variable Importance}

This can be time-consuming.

Algorithm looks like deep learning paper.

Boxplot for top-10 variables.
...
\caption{Outlier detection in \texttt{enpls}} \label{fig:diagram_enpls_od}
\end{figure}
...
<<enpls.od,fig.cap='Outlier detection result of the \\texttt{alkanes} dataset',fig.width=7, fig.height=7>>=
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{fig/diagram-enpls-en.pdf}
\caption{Ensemble modeling in \texttt{enpls}} \label{fig:diagram_enpls_en}
\end{figure}
...
For fitting ensemble partial least squares regression models, use \verb|enpls.fit()|:
...
<<predict.enpls.fit,fig.cap='Experimental values vs. predicted values'>>=
y.pred = predict(fit, newx = x)
df = data.frame(y, y.pred)

library("ggplot2")
ggplot(df, aes(x = y, y = y.pred)) +
...
Figure \ref{fig:predict.enpls.fit} shows the experimental values and predicted values.
